...
function EventListenerReturn OnUnitDamage(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData) // Making it so units that only take shield hp damage, or are above Half their HP don't count towards the wounded event
{
	...
	if (PreviousState.LowestHP > (UnitState.GetMaxStat(eStat_HP) / 2)) // Is above Half HP - Allows units to "Tank" without being penalized
		return ELR_NoInterrupt;
...
static function ProceedToNextRung( )
{
	...
	local StateObjectReference LocalPlayerReference, OriginalPlayerReference, UnitStateRef;
	...
	local int SoldierIndex, CampaignSaveID, EndingIndex;
